...
class EncoderDecoder(Chain):
    ...
    def __init__(self, vsize_enc, vsize_dec,
        ...
        if attn > 0:
            # add a linear layer to compute the context vector
            self.add_link("attention_context", L.Linear(4*n_units, 2*n_units))
    ...
    def feed_lstm(self, word, embed_layer, lstm_layer_list, train):
        ...
        embed_id = F.dropout(embed_layer(word), ratio=0.2)
        # feed into first LSTM layer
        hs = F.dropout(self[lstm_layer_list[0]](embed_id), ratio=0.2)
    ...
    def encode_decode_train(self, in_word_list, out_word_list, train=True):
        ...
        for next_word_var in var_dec[1:]:
            ...
            if self.attn == NO_ATTN:
                ...
                #Add attention
                dot_score = F.matmul(enc_states, self[self.lstm_dec[-1]].h, transb = True)
                alpha_list = F.softmax(F.transpose(dot_score))
                context_vector = F.matmul(alpha_list, enc_states)
                concat_vector = F.concat((self[self.lstm_dec[-1]].h, context_vector), axis = 1)
    ...
    def decoder_predict(self, start_word, enc_states, max_predict_len=MAX_PREDICT_LEN, sample=False):
        ...
        while pred_count < max_predict_len and (int(pred_word.data) != (EOS_ID)):
            ...
            if self.attn == NO_ATTN:
                ...
                predicted_out = self.out(self.attention_context(concat_vector))
                prob = F.softmax(predicted_out)
                alpha_arr = xp.concatenate((alpha_arr, alpha_list.data),axis=0)
